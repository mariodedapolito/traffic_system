...
class CarSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        Entities
            .WithoutBurst()
            .ForEach((int entityInQueryIndex, ref VehicleNavigation navigation, ref Translation translation, ref Rotation rotation, ref VehicleSpeed speed, in Car car, in VehiclePath vehiclePath, in LocalToWorld ltw) =>
            {
                NativeList<float4> path = vehiclePath.nodesList.Value;

                if (path.Length == 0 || navigation.needParking || navigation.isParked)
                {
                    return;
                }
                ...
                if (!navigation.intersectionStop)
                {
                    ...
                    {
                        if (path[navigation.currentNode][3] == PARKING_GATEWAY && (entityInQueryIndex + elapsedTime) % 5 == 0)
                        {
